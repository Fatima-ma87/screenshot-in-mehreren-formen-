...
def site_login(url, driver):
    ...
    driver.find_element(By.XPATH, 'xxxxxxxx').send_keys(login['username'], Keys.TAB)
    driver.find_element(By.XPATH, 'xxxxxxx').send_keys(login['password'])
    driver.find_element(By.XPATH, 'xxxxxxxxxx').submit()
    driver.find_element(By.XPATH, 'xxxxxxxxxx').send_keys(suche["muster"], Keys.ENTER) 
...
open_url('https://www.xxxxxxxx ', suche["muster"] + '-' + time.strftime('.%Y%m%d-%H%M-%S')) 
